...
-- 2024-08-30 Wyatt Best:	Set APPLICATION_FLAG to 'Y' if necessary.
-- =============================================
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			FROM ACADEMIC A
			INNER JOIN [custom].academickey AK ON AK.PEOPLE_CODE_ID = A.PEOPLE_CODE_ID
		...
		IF NOT EXISTS (
				...
				INNER JOIN [custom].AcademicKey AK ON AK.RecruiterApplicationId = RA.RecruiterApplicationId
				WHERE RA.ApplicationNumber = @SlateAppGuid
				)
			UPDATE AK
			SET AK.RecruiterApplicationId = RA.RecruiterApplicationId
			FROM [custom].AcademicKey AK
			INNER JOIN RecruiterApplication RA ON RA.ApplicationNumber = @SlateAppGuid
